      ...
      try {
        ...
        if (!response.ok) {
          ...
          if (response.status === 409) {
            // 409 ALREADY_EXISTS is not an error we should retry
...
This manual step is required because Firestore needs you to choose security rules.`);
  ...
  private async createAPIGateway() {
    ...
    if (!apiExists) {
      ...
      // The managed service name is generated by Google and we need to wait for it
      console.log('Waiting for API to be fully initialized with managed service name...');
      let apiReady = false;
      let checkCount = 0;
      const maxChecks = 10; // up to 5 minutes
      
      while (!apiReady && checkCount < maxChecks) {
        try {
          ...
          if (apiDetails && apiDetails.managedService) {
            console.log(`✅ API ready with managed service: ${apiDetails.managedService}`);
            apiReady = true;
          } else {
            checkCount++;
            console.log(`API details retrieved but no managed service yet (${checkCount}/${maxChecks})...`);
          }
        } catch (err: any) {
          checkCount++;
          if (checkCount < maxChecks) {
            console.log(`Error getting API details (${checkCount}/${maxChecks}): ${err.message}`);
          } else {
            throw err;
          }
        }
      ...
      if (!apiReady) {
        throw new Error('API creation succeeded but managed service name not available after 5 minutes');
      }
    } else {
      // API already exists, just get the details
      try {
        apiDetails = await this.gcpApiCall(
          `https://apigateway.googleapis.com/v1/projects/${this.config.projectId}/locations/global/apis/${apiId}`
        );
    ...
    if (!apiDetails || !apiDetails.managedService) {
      ...
      // Retry getting API details with longer waits
      let detailsRetries = 0;
      const maxDetailsRetries = 5;
      
      while (detailsRetries < maxDetailsRetries) {
        try {
          apiDetails = await this.gcpApiCall(
            `https://apigateway.googleapis.com/v1/projects/${this.config.projectId}/locations/global/apis/${apiId}`
          );
          if (apiDetails && apiDetails.managedService) {
            console.log(`✅ Retrieved managed service name: ${apiDetails.managedService}`);
            break;
          }
        } catch (err: any) {
          detailsRetries++;
          if (detailsRetries < maxDetailsRetries) {
            console.log(`Failed to get API details (attempt ${detailsRetries}/${maxDetailsRetries}). Waiting 30s...`);
            await new Promise(resolve => setTimeout(resolve, 30000));
          }
        }
      }
      ...
      if (!apiDetails || !apiDetails.managedService) {
        // If we still can't get details, we need to fail - we can't proceed without the correct managed service name
        throw new Error(`Failed to retrieve API details after ${maxDetailsRetries} attempts. The API may still be initializing. Please try again in a few minutes.`);
      }
    }
